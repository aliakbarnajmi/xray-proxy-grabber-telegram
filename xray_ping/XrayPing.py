...
BYPASS_MODE_PORT = randint(50000, 60000)
...
def real_delay(port: int, proxy_name: str):
    ...
    try:
        ...
        country = requests.get("https://myip.wtf/json", timeout=10, proxies=dict(http=proxy, https=proxy)).json().get("YourFuckingCountryCode")
    ...
    return dict(proxy=proxy_name, realDelay_ms=round(delay if delay <= 0 else delay * 1000), is403=(statusCode == 403), country = country)
...
class XrayPing:
    ...
    def __init__(self, configs: list[str], limit: int = 200) -> None:
        ...
        runXrayThread = Thread(target=subprocess.run,
                               args=([Path(f"xray_ping/{xray_file}").resolve(), "run", "-c", configFilePath],))
        ...
        if real_delay(BYPASS_MODE_PORT, "bypass_mode")["realDelay_ms"] < 0:
            print("***************************************************************************")
            print(confFinalStr)
